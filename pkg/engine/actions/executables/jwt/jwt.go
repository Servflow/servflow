...
func init() {
	fields := map[string]actions.FieldInfo{
		...
		"field": {
			Type:        actions.FieldTypeString,
			Label:       "Field",
			Description: "Field name for token data",
			Required:    true,
		},
		"key": {
			Type:        actions.FieldTypeString,
			Label:       "Key",
			Description: "JWT signing/verification key (PEM or secret)",
			Required:    false,
		},
		"jwksURL": {
			Type:        actions.FieldTypeString,
			Label:       "JWKS URL",
			Description: "URL to fetch JSON Web Key Set for verification",
			Required:    false,
		},
		"claims": {
			Type:        actions.FieldTypeMap,
			Label:       "Claims",
			Description: "JWT claims as key-value pairs",
			Required:    false,
		},
		"failOnValidationError": {
			Type:        actions.FieldTypeBoolean,
			Label:       "Fail on Validation Error",
			Description: "Treat JWT validation failures as workflow failures",
			Required:    false,
			Default:     true,
		},
	}
